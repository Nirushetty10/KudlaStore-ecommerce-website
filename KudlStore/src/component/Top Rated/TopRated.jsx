...
const TopRated = () => {
  ...
  const {data,error, loading, refetch} = useFetch(`${config.url}/api/product/toprated`);

  const viewTopRatedProductsHandler = () => {
    navigate("/top-rated");
  }
  return (
    <div className={classes.container}>
        ...
        <div className={classes.category_title}>
            <h3>Top Rated</h3>
            <button className={classes.view_all_button} onClick={viewTopRatedProductsHandler}>VIEW ALL</button>
        </div>
        <div className={classes.category_items}>
           {data.map(item => {
             return <ItemCard key={item.id} item={item} color="blue"/>
           })}
        </div>
        </div>
    </div>
